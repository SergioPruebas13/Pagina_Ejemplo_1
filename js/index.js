...
function cargarDatos(){
  $.ajax({
    url: "https://sergiopruebas13.github.io/Pagina_Ejemplo_1/datos/info.json"
            ...
            Cargar_Eventos_Proximos();
           Cargar_Eventos_Pasasdos();
            
});
}

function Cargar_Eventos_Proximos(){
  var num = 0;
  eventos_proximos.sort(function (a, b) {// para ordenar el array de los eveentos mas proximos a los mas viejos 
    ...
    return 0;
      ...
      for (let i = 0; i < eventos_proximos.length; i++) {
//<a </a>
        if(num!=2){
          if(eventos_proximos[i].fecha_ > fecha_Actual){
           var Fecha = Buscar_Fecha(eventos_proximos[i].fecha_);
           var id_E = eventos_proximos[i].id_;
           //console.log(Fecha)
